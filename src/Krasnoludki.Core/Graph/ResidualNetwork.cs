...
public class ResidualNetwork
{
    ...
    public IGraphNode GetNode(int id)
    {
        ...
        throw new ArgumentOutOfRangeException(nameof(id), $"ArgumentOutOfRangeException: Node with id: {id} doesn't exists in residual network!\n");
    }
    ...
    public ResidualNetwork(List<GraphDwarf> dwarves, List<GraphMine> mines)
    {
        ...
        foreach(GraphDwarf dwarf in dwarves)
        {
            ...
            foreach(GraphMine mine in mines) //Edges from Dwarf to Mines with preferred mineral
            {
                ...
                // (in this case rounded to 5 digits after comma to prevent problems with computer precision)
